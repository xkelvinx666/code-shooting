  ...
  return (
    <>
      ...
      <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 shadow z-10 bg-white p-4 rounded-l max-w-2xl">
        <SelectOptions<Map> name="map" active={selectedMap} options={mapOptions} onClick={(val) => setSelectedMap(val)} />
        <SelectOptions<Job> name="character" active={characterJob} options={jobOptions} onClick={(val) => setCharacterJob(val)} />
        ...
        <Link href="/game">
          <a className="block mx-auto bg-blue-400 text-white text-center h-10 leading-10 w-40" onClick={handleLinkClick}>enter game</a>
        </Link>
      </div>
    </>
  );
